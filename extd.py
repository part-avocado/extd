...
# No System Exit, instead return error code
def err(err_code, add_msg=None):
  if err_code == 400:
    ...
    print(
        f"[EXTD] Oops... Developers Were Lazy, No Error Code Provided (None) \nDeveloper Note: {add_msg}"
    )
  elif err_code == 500:
    ...
    print(
        f"[EXTD] Oops... Microsoft Parental Controls Blocked This... (450) \nDeveloper Note: {add_msg}"
    )
    return err_code
  elif err_code == 529:
    ...
    print(
        f"[EXTD] Oops... Generic Error. See project GitHub for more details. ({err_code}) \nDeveloper Note: {add_msg}"
    )
    return err_code
